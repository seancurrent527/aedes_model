...
def match_peaks(offsets, num_fake, num_real):
    ...
    if num_real == num_fake:
        distance = lambda off: abs(off[0]) + abs(off[1])
        ordering = sorted(offsets, key = lambda x: distance(offsets[x]))
        matching = {}
        chosen_offsets = []
        for real, fake in ordering:
            if real not in matching.keys():
                if fake not in matching.values():
                    matching[real] = fake
                    chosen_offsets.append(offsets[(real, fake)])
        return zip(*chosen_offsets)

    else: #num_real != num_fake:
    '''
...
def main():
    ...
    on_table.to_csv('tables/'+ directory + '/revised/' + modname + '_D_on_table.csv')
    off_table.to_csv('tables/'+ directory + '/revised/' + modname + '_D_off_table.csv')
    with open('tables/'+ directory + '/revised/' + modname + '_latex.txt', 'w') as fp:
